        ...
        when (arguments?.getInt(PAGE_NUMBER)) {
            ...
            1 -> {  //  save the date
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                    place_holder_base_layout.setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.screen1bg))
                } else {
                    place_holder_base_layout.background = ContextCompat.getDrawable(context, R.drawable.screen1bg)
                }
            ...
            3 -> {  //  details base layout
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                    place_holder_base_layout.setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.screen3bg))
                } else {
                    place_holder_base_layout.background = ContextCompat.getDrawable(context, R.drawable.screen3bg)
                }
